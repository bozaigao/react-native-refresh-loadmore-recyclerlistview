...
//#endif
    ...
    refreshRequestDebouncer = debounce((executable: () => void) => {
        executable();
    });
...
export default class RecyclerListView extends React.Component<RecyclerListViewProps, RecyclerListViewState> {
    ...
    private _onSizeChanged(layout: Dimension): void {
        const hasHeightChanged = this._layout.height !== layout.height,
            hasWidthChanged = this._layout.width !== layout.width;
    ...
    private _renderRowUsingMeta(itemMeta: RenderStackItem): JSX.Element | null {
        ...
        if (!ObjectUtil.isNullOrUndefined(dataIndex) && dataIndex < dataSize) {
            const itemRect = (this._virtualRenderer.getLayoutManager() as LayoutManager).getLayouts()[dataIndex],
                data = this.props.dataProvider.getDataForIndex(dataIndex),
                type = this.props.layoutProvider.getLayoutTypeForIndex(dataIndex);
    ...
    private _processOnEndReached(): void {
        if (this.props.onEndReached && this._virtualRenderer) {
            const layout = this._virtualRenderer.getLayoutDimension(),
                windowBound = this.props.isHorizontal ? layout.width - this._layout.width : layout.height - this._layout.height,
                viewabilityTracker = this._virtualRenderer.getViewabilityTracker(),
                lastOffset = viewabilityTracker ? viewabilityTracker.getLastOffset() : 0;
...
RecyclerListView.propTypes = {
    ...
    //Enables animating FindRecyclerListView item cells e.g, shift, add, remove etc. This prop can be used to pass an external item animation implementation.
